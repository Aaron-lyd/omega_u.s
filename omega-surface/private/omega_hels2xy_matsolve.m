...
sx(isnan(sx)) = 0; % DEV.  Will be handled by goodx having a sx ~= 0 condition, later.
sy(isnan(sy)) = 0; % DEV.  Will be handled by goody having a sy ~= 0 condition, later.
...
val = [-p1x ./ DXCm .* sqrtAREAXm; -p1y ./ DYCm .* sqrtAREAYm;...
        p1x ./ DXCm.* sqrtAREAXm;   p1y ./ DYCm .* sqrtAREAYm];
...
if STRAT
    ...
    im = p1x .* H_imh_m ./ DXCm .* sqrtAREAXm;
    im(~goodxm) = 0; % Ignore invalid connections
    ip = im;
    jm = p1y .* H_jmh_m ./ DYCm .* sqrtAREAYm;
...
mat_s2xy = sparse(row(good_1p5), col(good_1p5), val(good_1p5), neq, N );
