function [dz, ehelTz, N] = omega_hel_Tz_matsolve(z, Z, SppZ, TppZ, Tz, Tzz, u, v, uz, vz, sx, sy, dzi, dzj, DXG, DYG, DXC, DYC, RAX, RAY, RAC, sqrtAREA, i0, j0, I0, A4, A5, OPTS)
...
val = [-jm(m)'; -im(m)'; ip(m)'; jp(m)'; zeros(1, N)] ./ (2 * sqrtAREA(m)') .* Tz(m)';
...
%     val2 = [jm(m)'; im(m)'; ip(m)'; jp(m)'; zeros(1,N)] ./ (2 * RAC(m)');
    val2 = [jm(m)'; im(m)'; ip(m)'; jp(m)'; zeros(1,N)] ./ (4 * sqrtAREA(m)') .* Tz(m)';
    val2(~good) = 0; % Ignore invalid connections
    val2(5,:) = sum(val2(1:4,:), 1); % note + sign
    val = val + val2;
end
...
%     val2 = [jm(m)'; im(m)'; ip(m)'; jp(m)'; zeros(1,N)] ./ (2 * RAC(m)');
    val2 = [jm(m)'; im(m)'; ip(m)'; jp(m)'; zeros(1,N)] ./ (2 * sqrtAREA(m)') .* Tz(m)';
    val2(~good) = 0; % Ignore invalid connections
    val2(5,:) = sum(val2(1:4,:), 1); % note + sign
    val = val + val2;
end
