function [dz, s2, N] = omega_s2_matsolve2(z, Z, SppZ, TppZ, sx, sy, dzi, dzj, DXG, DYG, RAC, sqrtAREA, i0, j0, I0, A4, A5, OPTS)
...
% sx(isnan(sx)) = 0; % DEV.  Will be handled by goodx having a u ~= 0 condition, later.
% sy(isnan(sy)) = 0; % DEV.  Will be handled by goody having a v ~= 0 condition, later.
...
%% New pinning
mr = remap(i0,j0);
mat(N+1,mr) = rms(val(good));
rhs = zeros(N+1,1);
rhs(1:N) = -s2(m).* sqrtAREA(m);
