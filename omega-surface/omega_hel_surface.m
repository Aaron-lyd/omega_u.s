function [zns, sns, tns, ehel, d] = omega_hel_surface(S, T, Z, U, V, z, OPTS)
% Create a series of omega surfaces, users can select the mode (in the the options)
% to choose what surface to create, OPTS.MODE =
% 1. omega_hel surface, minimizes the spurious diasurface velocity; 
% 2. omega_s surface, minimizes the slope error;
% 3. omega_1.5 surface, minimizes e^hel_x and e^hel_y on the u and v cell
% of the c-grid;
% 4. omega_s^2, minimizes the squared slope error at the tracer cell;
% 5. combine omega_hel (MODE=1) and omega_s^2 (MODE=4);
% 6. omega_s2xy, minimizes the squared slope error at the u and v cell;
...
%
% [p, s, t] = omega_hel_surface(S, T, P, p, ref_cast, WRAP)
...
% ehel [ni, nj]: e^hel on omega surface
% diags [struct]: diagnostics such as clock time and norms of slope
...
%
% Lang, Stanley, McDougall: Forming approximately neutral surfaces with
%  minimal effect of neutral helicity on the diasurface velocity.
%
...
for iter = 1 : n_iter
  ...
  elseif mode == 5 % combining omega_hel and omega_s^2
      [dz, ehel, N] = omega_hels2_matsolve(z, Z, SppZ, TppZ, u, v, uz, vz, sx, sy, dzi, dzj, DXG, DYG, RAX, RAY, RAC, sqrtAREA, i0, j0, I0, A4, A5, OPTS);
