function out = rms_Cgrid(T, U, V, AREA_IJ, AREA_iJ, AREA_Ij)
...
out = sqrt( ...
  (nansum(flat(AREA_IJ .* T.^2)) + nansum(flat(AREA_iJ .* U.^2)) + nansum(flat(AREA_Ij .* V.^2))) ./ ...
  (sumgood(AREA_IJ, T) + sumgood(AREA_iJ, U) + sumgood(AREA_Ij, V)) );
