function [gamma_n, SP_ntp, t_ntp, T_ntp, p_ntp,...
          gamma_ntp1, gamma_ntp2, gamma_ntp3, gamma_ntp4,...
          SP_ntp1, t_ntp1, T_ntp1, p_ntp1, SP_ntp2, t_ntp2, T_ntp2, p_ntp2,...
          SP_ntp3, t_ntp3, T_ntp3, p_ntp3, SP_ntp4, t_ntp4, T_ntp4, p_ntp4] = eos80_legacy_gamma_scv_labelling_STP_GJS_YL(SP,t,p,long,lat)
...
% end
ref_data = 'ref_data.mat';
ref_data_file = which(ref_data);
load(ref_data_file,'gamma_n_ref','lat_ref','long_ref','p_ref',...
        'a_ref','SP_ref','t_ref','T_ref')
...
edges = p_ref;
edges(end) = inf;
[~, Iz1] = histc(p_ntp1, edges); % edges(Iz1) <= p_ntp1 < edges(Iz1+1)
[~, Iz2] = histc(p_ntp2, edges);
[~, Iz3] = histc(p_ntp3, edges);
[~, Iz4] = histc(p_ntp4, edges);
clearvars edges
...
% I4z_nn = Iz4 > 0;
% Ixyz4 = sub2ind(size(SP_ref),Iz4(I4z_nn).',Iy0(I4z_nn)+1,Ix0(I4z_nn));
% End GJS bug fix, 26/03/2019

% calculate a gamma for the bottle to each of the 4 surrounding grid points
% Lang's version replacement begin{
gamma_ntp1 = NaN(1,length(SP));
for I = find(Iz1 > 0)
  i = Ix0(I);
  j = Iy0(I);
  k = Iz1(I);
  gamma_ntp1(I) = gamma_n_eq_13_JM_1997_v2(gamma_n_ref(:,j,i), SP_ref(:,j,i), T_ref(:,j,i), p_ref,k, p_ntp1(I));
end
...
for I = find(Iz2 > 0)
  ...
  j = Iy0(I);
  k = Iz2(I);
  gamma_ntp2(I) = gamma_n_eq_13_JM_1997_v2(gamma_n_ref(:,j,i), SP_ref(:,j,i), T_ref(:,j,i), p_ref,k, p_ntp2(I));
end
...
for I = find(Iz3 > 0)
  ...
  j = Iy0(I)+1;
  k = Iz3(I);
  gamma_ntp3(I) = gamma_n_eq_13_JM_1997_v2(gamma_n_ref(:,j,i), SP_ref(:,j,i), T_ref(:,j,i), p_ref,k, p_ntp3(I));
end
...
for I = find(Iz4 > 0)
  ...
  j = Iy0(I)+1;
  k = Iz4(I);
  gamma_ntp4(I) = gamma_n_eq_13_JM_1997_v2(gamma_n_ref(:,j,i), SP_ref(:,j,i), T_ref(:,j,i), p_ref,k, p_ntp4(I));
end
% } end
...
%             eos80_legacy_gamma_n_labelling_error(SP(Inn).',t(Inn).',p(Inn).',lat(Inn).',...
%             SP_ref(:,I3(Inn)),t_ref(:,I3(Inn)),p_ref3_dummy(:,Inn),gamma_n_ref(:,I3(Inn)),a_ref(:,I3(Inn)),...
