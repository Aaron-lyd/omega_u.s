function [gamma_n, gamma_error_lower, gamma_error_upper, SP_ntp, T_ntp, p_ntp] = eos80_gamma_n(SP,t,p,long,lat)
...
if nargout > 1
    ...
    [gamma_n(Iocean), gamma_error_lower(Iocean), gamma_error_upper(Iocean), SP_ntp(Iocean), T_ntp(Iocean), p_ntp(Iocean)] = ...
        eos80_gamma_n_labelling(SP(Iocean),t(Iocean),p(Iocean),long(Iocean),lat(Iocean));
else
    gamma_n(Iocean) = eos80_gamma_n_labelling(SP(Iocean),t(Iocean),p(Iocean),long(Iocean),lat(Iocean));
end
