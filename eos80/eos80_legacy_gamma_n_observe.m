function [gamma_n, gamma_error_lower, gamma_error_upper] = eos80_legacy_gamma_n_observe(SP,t,p,long,lat)
...
if nargout > 1
    ...
    [gamma_n(Iocean), gamma_error_lower(Iocean), gamma_error_upper(Iocean)] = ...
        eos80_legacy_gamma_n_labelling_observe(SP(Iocean),t(Iocean),p(Iocean),long(Iocean),lat(Iocean));
else
    gamma_n(Iocean) = eos80_legacy_gamma_n_labelling_observe(SP(Iocean),t(Iocean),p(Iocean),long(Iocean),lat(Iocean));
end
