...
[gscvE, ~, ~, SE, tE, TE, pE] = eos80_legacy_gamma_n_STP_GJS_no_extrapolation(SB, tB, pB, longOCCAzxy, latOCCAzxy);
...
%%
[sx, sy] = ntp_slope_error(SppZ, TppZ, ZB, pscv, 1e-8, g.DXCvec, g.DYCsc, [1;0]);
...
Df_south_rms = 1000*[detailpt.s2_st_rms; detailns.s2_st_rms; detailns_s.s2_st_rms; detail_hel.s2_st_rms; detail_helTz.s2_st_rms; detail_helSz.s2_st_rms; detailns_s2xy.s2_st_rms; detail_hel_s2xy.s2_st_rms];
...
data_rms = [root_mean_square(z_sigma-zns), rms(-zns(~isnan(zns))), ...
...
figure
ax1 = subaxis(1,1,1, OPTS_AXES{:});
hh = quivers(x_quiver, y_quiver, u_hel, v_hel,5,3,'m/s','k');
hold on
hf = fig_map(ax1, g.XCvec, g.YCvec, ehel_hel, land_OCCA, OPTS_FIGS);
pcol_logsigned(ax1, g.XCvec, g.YCvec, ehel_hel, -10, -7);
hold off
