...
ehel_south_rms = [dpt.e_hel_st_rms; dns.e_hel_st_rms; dns_s.e_hel_st_rms; dhel.e_hel_st_rms; dhelTz.e_hel_st_rms; dhelSz.e_hel_st_rms; dns_s2xy.e_hel_st_rms; dhel_s2xy.e_hel_st_rms];
Df_south_rms = 1000*[dpt.s2_st_rms; dns.s2_st_rms; dns_s.s2_st_rms; dhel.s2_st_rms; dhelTz.s2_st_rms; dhelSz.s2_st_rms; dns_s2xy.s2_st_rms; dhel_s2xy.s2_st_rms];
...
data = cat(3, ehel_ortho, ehel_topo, ehel_scv);
data_rms = [ehel_ortho_rms, ehel_topo_rms, ehel_scv_rms];
                                                  
% ehelpt_rms = rms_Cgrid(ehelpt, 0, 0, g.RACvec, 0, 0);
                                                   
quantity_txt = '$\\bf{u} \\cdot \\bf{s}$ on the ';        
rms_txt = 'RMS of $\\bf{u} \\cdot \\bf{s}$ = %.2d m/s';        
txt1 = ['(a) ', quantity_txt,'Orthobaric-surface, ', rms_txt];
txt2 = ['(b) ', quantity_txt,'topobaric-surface, ', rms_txt];
txt3 = ['(c) ', quantity_txt,'neutral density surface, ', rms_txt];

title_text = char(txt1,txt2,txt3);
fig_hf = fig_map_plotting(data, data_rms, title_text, OPTS_plot, OPTS_FIGS);
...
data_rms = [dpt.ehelTz_rms,dns.ehelTz_rms,dns_s.ehelTz_rms,dhel.ehelTz_rms,dhelTz.ehelTz_rms,dhelSz.ehelTz_rms,dns_s2xy.ehelTz_rms,dhel_s2xy.ehelTz_rms];
...
data_rms = [dpt.ehelSz_rms,dns.ehelSz_rms,dns_s.ehelSz_rms,dhel.ehelSz_rms,dhelTz.ehelSz_rms,dhelSz.ehelSz_rms,dns_s2xy.ehelSz_rms,dhel_s2xy.ehelSz_rms];
...
data = cat(3, log10(df_pt), log10(df_ns), log10(df_ns_s), log10(df_hel),...
                                   log10(df_helTz), log10(df_helSz), log10(df_ns_s2xy), log10(df_hel_s2xy));
data_rms = [df_pt_rms, df_ns_rms, df_ns_s_rms, df_hel_rms, df_helTz_rms,...
                                              df_helSz_rms, df_ns_s2xy_rms, df_hel_s2xy_rms];
...
%% Figure 5:Depth map
figure('Position', [0 0 1200 1000])
OPTS_plot.n_lim = [-70,   -1800, -10, -20, -20, -20, -10, -30];
OPTS_plot.p_lim = [70,     0,  10,  20,  20,  20,  10,  30];
OPTS_plot.bwr = [1, 0, 1, 1, 1, 1, 1, 1];
OPTS_plot.log = 0;
...
data_rms = [dpt.ad_df_rms, dns.ad_df_rms, dns_s.ad_df_rms, dhel.ad_df_rms, dhelTz.ad_df_rms,...
                                       dhelSz.ad_df_rms,dns_s2xy.ad_df_rms,dhel_s2xy.ad_df_rms];
...
%% Extra scatter: The scatter of e^hel and s^2 for omega_hel and omega_hel+s^2
OPTS_AXES = {'Margin', .06, 'Spacing', .09};
figure('Position', [0 0 1800 1000])

ax1 = subaxis(2,2,1, OPTS_AXES{:});
good = ~isnan(ehel_topo) & ~isnan(df_topo);
xvec = ehel_topo(good);
yvec = df_topo(good);
hist2D(ax1,log10(abs(xvec)), log10(abs(yvec*1e-3)),'nbins',400, 'RescaleFcn', @(x) sqrt(x))
plot(log10(ehel_topo_rms), log10(dtopo.s2_rms), 'Marker', '+', 'Color', [0 0 0], 'MarkerSize', 20, 'LineWidth', 3)
patch_x = [-12 log10(3e-9) log10(3e-9) -12];
patch_y = [log10(1e-8) log10(1e-8) -20 -20];
blue = [0.5843 0.8157 0.9882];
patch(patch_x, patch_y, -1 + zeros(length(patch_x),1), blue, 'EdgeColor', 'none')
...
txt_y = '$log_{10} (\\bf{s}^2)$, RMS = %.2d';
ylabel(sprintf(txt_y ,dpt.s2_rms) , 'fontsize',10,'Interpreter','latex');
title('(a) topobaic-surface', 'fontsize',10,'Interpreter','latex')
set(gca,'fontsize', 15);

ax2 = subaxis(2,2,2, OPTS_AXES{:});
good = ~isnan(ehel_ortho) & ~isnan(df_ortho);
xvec = ehel_ortho(good);
yvec = df_ortho(good);
hist2D(ax2,log10(abs(xvec)), log10(abs(yvec*1e-3)),'nbins',400, 'RescaleFcn', @(x) sqrt(x))
plot(log10(ehel_ortho_rms), log10(dortho.s2_rms), 'Marker', '+', 'Color', [0 0 0], 'MarkerSize', 20, 'LineWidth', 3)
patch_x = [-12 log10(3e-9) log10(3e-9) -12];
patch_y = [log10(1e-8) log10(1e-8) -20 -20];
patch(patch_x, patch_y, -1 + zeros(length(patch_x),1), blue, 'EdgeColor', 'none')
grid on
...
txt_y = '$log_{10} (\\bf{s}^2)$, RMS = %.2d';
ylabel(sprintf(txt_y ,dns.s2_rms) , 'fontsize',10,'Interpreter','latex');
title('(b) orthobaric-surface', 'fontsize',10,'Interpreter','latex')
set(gca,'fontsize', 15);
...
txt_y = '$log_{10} (\\bf{s}^2)$, RMS = %.2d';
ylabel(sprintf(txt_y ,dhel.s2_rms) , 'fontsize',10,'Interpreter','latex');
title('(c) neutral density surface', 'fontsize',10,'Interpreter','latex')
set(gca,'fontsize', 15);
